"""Evaluate smooth mobility models with configurable pauses."""
...
from mobility_models import SmoothMobilityWithPause
...
def run_smooth_mobility(
    ...
    mobility = SmoothMobilityWithPause(
        area_size,
        min_speed,
        max_speed,
        pause_mean=pause,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Smooth mobility sweep")
    parser.add_argument("--avg-speeds", default="1.5,3.0", help="Comma separated average speeds (m/s)")
    ...
    parser.add_argument("--speed-variation", type=float, default=0.3)
    ...
    parser.add_argument(
        "--output",
        default=os.path.join(RESULTS_DIR, "mobility_smooth.csv"),
        ...
        for pause in pauses:
            for density in densities:
                for rep in range(args.replicates):
                    seed = args.seed + rep + combo_index * 199
                    row = run_smooth_mobility(
                        avg_speed,
                        pause,
                        density,
