...
def plot_energy_breakdown(
    ...
    ax.bar(indices, idle, width=width, bottom=tx + rx, label="Veille")

    ax.set_xticks(indices)
    ax.set_xticklabels([f"Classe {name}" for name in classes])
    ax.set_ylabel("Énergie moyenne par nœud (J)")
    ax.set_title("Répartition de l'énergie par classe")
...
def plot_pdr_comparison(
    ...
    fig, ax = plt.subplots()
    ax.bar(indices - width / 2, uplink, width=width, label="PDR uplink")
    ax.bar(indices + width / 2, downlink, width=width, label="PDR downlink")

    ax.set_xticks(indices)
    ax.set_xticklabels([f"Classe {name}" for name in classes])
    ax.set_ylabel("Taux de livraison (%)")
    ax.set_ylim(0, 105)
    ax.set_title("Comparaison des PDR uplink/downlink")
    ax.grid(True, axis="y", linestyle="--", linewidth=0.5, alpha=0.6)
    ax.legend()
    fig.tight_layout()
